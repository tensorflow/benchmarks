...
if [ "$1" = "multigpu" ]; then
  mpiexec -n 4 python benchmarks/scripts/keras_benchmarks/run_benchmark.py --keras_backend=$KERAS_BACKEND --gpu_count='4' --gpu_platform='NVIDIA Tesla K80' --cpu_num_cores='8' --cpu_memory='30' --cpu_memory_info='GB' --platform_type='GCP' --platform_machine_type='n1-standard-8'
fi